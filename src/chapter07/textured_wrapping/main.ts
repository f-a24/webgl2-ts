...
const configure = () => {
  ...
  texture.setImage(
    'https://raw.githubusercontent.com/oreilly-japan/real-time-3d-graphics-with-webgl2-2e-ja/master/common/images/webgl-marble.png'
  );
};
...
const initControls = () => {
  const gui = new GUI();
  const wrapOptions = ['CLAMP_TO_EDGE', 'REPEAT', 'MIRRORED_REPEAT'] as const;
  const textureWrapAxis = ['TEXTURE_WRAP_S', 'TEXTURE_WRAP_T'] as const;
  textureWrapAxis.forEach(axis => {
    gui
      .add({ [axis]: wrapOptions[1] }, axis, wrapOptions)
      .onChange((v: (typeof wrapOptions)[number]) => {
        gl.bindTexture(gl.TEXTURE_2D, texture.glTexture);
